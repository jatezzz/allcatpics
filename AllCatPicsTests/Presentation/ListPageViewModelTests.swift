...
class ListPageViewModelTests: XCTestCase {
    ...
    func testViewModel_loadsNextPageSuccessfully() {
        ...
        mockRepository.catsToReturn = [.make(tags: ["tag1"], id: "123")]
    ...
    func testViewModel_doesNotLoadNextPageWhileAlreadyLoading() {
        ...
        XCTAssertTrue(mockRepository.didAttemptToFetchCats == false,
                      "Should not attempt to load next page while already loading")
    }

    func testViewModel_shouldLoadMoreDataReturnsCorrectValue() {
        viewModel.cats = [.make(id: "1")]

        let lastCat = viewModel.cats.last!
        XCTAssertTrue(viewModel.shouldLoadMoreData(currentItem: lastCat),
                      "Should load more data when the current item is the last item")

        let notLastCat = Cat(tags: [], createdAt: nil, updatedAt: nil, mimetype: nil, size: nil, id: "2", editedAt: nil)
        XCTAssertFalse(viewModel.shouldLoadMoreData(currentItem: notLastCat),
                       "Should not load more data when the current item is not the last item")
    }
}
